...
function prepare_host_noninteractive() {
	...
	[[ ! -f "${USERPATCHES_PATH}"/lib.config && -f "${SRC}"/config/templates/lib.config ]] && run_host_command_logged cp -pv "${SRC}"/config/templates/lib.config "${USERPATCHES_PATH}"/lib.config
